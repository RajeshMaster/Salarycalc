...
$(document).ready(function(){
	...
	jQuery.validator.addMethod("accessDateCheck", function(value, element, params) {
		if (!/Invalid|NaN/.test(new Date(value))) {
			return new Date(value) > new Date($(params).val());
		}
		return isNaN(value) && isNaN($(params).val()) 
	...
	jQuery.validator.addMethod("lessThanStartdate", function(value, element, params) {
		if (!/Invalid|NaN/.test(new Date(value))) {
			return new Date(value) <= new Date($(params).val());
		}
		return isNaN(value) && isNaN($(params).val()) 
	...
	// Madasamy 08/11/2020
	// Jquery CUstom Less than Joining Date Validation Check
	jQuery.validator.addMethod("lessThanJoindate", function(value, element, params) {
		if (!/Invalid|NaN/.test(new Date(value))) {
			return new Date(value) <= new Date($(params).val());
		}
		return isNaN(value) && isNaN($(params).val()) 
		|| (Number(value) <= Number($(params).val())); 
	},'Must be less than Joining Date.');
	// End Jquery CUstom Less than Joining Date Validation Check
...
function fnMoneyFormatNegative(name,flg) {
	...
	fnMoneyFormatWithoutleadingzero(name, value,flg);
	var value = $('#'+name);
	if (isNumeric((value.val()).replace(/,/g,'')) && value.val() != '' && (value.val()).trim() != 0) {
		value.val('-' + value.val());
	}else{
		value.val('');
	}
