...
<script type="text/javascript">
		...
		$("#importform").validate({
			...
			submitHandler: function(form) { // for demo
				...
				if(confirmprocess) {
					pageload();
					if ($('#mainmenu').val() != "family") {
						$('#importform').attr('action', '../Family/importprocess?mainmenu');
					}
					form.submit();
					return true;
				} else {
...
{{ Form::open(array('name'=>'importform', 'id'=>'importform', 
                      'url' => 'Family/importprocess?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),
                      'method' => 'POST')) }}
     {{ Form::hidden('mainmenu', $request->mainmenu , array('id' => 'mainmenu')) }}
		...
		<fieldset style="width:98%;">
			<div class = "box50per ml150  mt15">
				<select id = "contentsel" name = "contentsel" size="30" style="height: 140px;margin-bottom: 13px;" ondblclick="return dblclick();" 
