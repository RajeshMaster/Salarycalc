	...
	<article id="expenses" class="DEC_flex_wrapper " data-category="expenses expenses_sub_3">
		...
		<div class="col-xs-12 pt5">
			...
			<div class="col-xs-6" style="text-align: left;margin-left: -15px;">
				...
				<a onclick="return confirmdownload();" href="{{ url('salarycalcplus/gensenDownload?empid='.$request->Emp_ID.'&lastname='.$request->lastname.'&selYear='.$request->selYear) }}" class="btn btn-success box190">
		          <span class="fa fa-file-excel-o"></span>&emsp;{{ trans('messages.lbl_gensen_download') }}
		        </a>

		        <!-- <a onclick="return confirmdownload();" href="{{ url('salarycalcplus/gensenDownloadNew?empid='.$request->Emp_ID) }}" class="btn btn-success box190">
		          <span class="fa fa-file-excel-o"></span>&emsp;{{ trans('messages.lbl_gensen_download') }} New
		        </a> -->
			...
			<table class="tablealternate CMN_tblfixed" style="width: <?php echo $count; ?>% !important;">
				...
				<thead class="CMN_tbltheadcolor">
					<tr>
						...
						<th rowspan="3" class="vam">{{ trans('messages.lbl_paymentdate') }}</th>
						@php($sal_det_cnt = '')
						@if(count($salary_det)!="")
			 				@for ($i = 0; $i < count($salary_det); $i++)
			 					@if(isset($temp_salaryDetails[$salary_det[$i]->Salarayid]) && $temp_salaryDetails[$salary_det[$i]->Salarayid] != '0')
						...
						<th rowspan="2" class="vam" style="width: 9%;background-color: #f39c1280 !important">{{ trans('messages.lbl_totpay_amt') }}</th>
						@endif
						@php($sal_ded_cnt = '')
	 					@if(count($salary_ded)!="")
		 				...
		 				<th rowspan="2" class="vam" style="width: 9%;background-color: #f39c1280 !important">{{ trans('messages.lbl_totded_amt') }}</th>
			 			@endif
			 			@php($sal_trv_cnt = '')
						@if($tot_travel_amt != '')
	 						@php($sal_trv_cnt += 1)
		...
		@endif
		{{ Form::close() }}
		{{ Form::open(array('name'=>'salarycalchistorydwnld',
							'id'=>'salarycalchistorydwnld',
							'url'=>'salarycalcplus/history?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),
							'files'=>true,
							'method' => 'POST' )) }}
			{{ Form::hidden('mainmenu', $request->mainmenu, array('id' => 'mainmenu')) }}
