...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#salamt").on("keydown keyup", function() {
...
<div class="CMN_display_block" id="main_contents">
	...
	<article id="expenses" class="DEC_flex_wrapper " data-category="expenses expenses_sub_7">
		@if(isset($detedit))
			{{ Form::model($detedit, ['name'=>'addeditcontractemp', 
		...
		@endif
			...
			<div class="col-xs-6">
				<fieldset class="col-xs-12 mt20 ml10" style="display: inline-block;">
					<legend align="left" style="width: auto !important; background-color: white; border:none !important;float: left; font-size: 15px; position: relative; margin-top: -13px;margin-bottom: 0px !important;margin-left: -5px !important;">
					...
					<div class="col-xs-12 pm0 pull-right mb10 pl20 pr10 mt10 fwb">
						...
						<span class="mr30">{{ trans('messages.lbl_year') }}</span>
						<span class="mr30" style="margin-left: 10px">
							<?php
							$month_array = array(
													'' => '',
													'1' => '1月分',
							...
							?>
							@if(isset($detedit))
								{{ Form::select('month', $month_array, null, 
								array('id' => 'month',
									'data-label' => trans('messages.lbl_monthnumb'))) }}
							@else
				...
				<fieldset class="col-xs-12 ml10">
					<legend align="left" style="width: auto !important; background-color: white; border:none !important;float: left; font-size: 15px; position: relative; margin-top: -13px;margin-bottom: 0px !important;margin-left: -5px !important;">
						<b>{{ trans('messages.lbl_salary_det') }}</b>
					</legend>
						...
						@if(count($salary_det)!="")
							...
							@for ($m = 0; $m < count($salary_det); $m++)
								<div class="col-xs-12 mt5 @if($m == $len - 1) mb10 @endif">
									...
									<div class="col-xs-5">
										{{ Form::text('salary_'.$salary_det[$m]->Salarayid,null, 
										array('id'=>'salary_'.$salary_det[$m]->Salarayid,
				...
				<!--  -->
				<fieldset class="col-xs-12 mt20 ml10">
				<legend align="left" 
			...
			<div style="float: right;" class="col-xs-6">
						...
						@if(count($salary_ded)!="")
							...
							@for ($n = 0; $n < count($salary_ded); $n++)
								<div class="col-xs-12 mt5 @if($n == $len1 - 1) mb10 @endif">
									...
									<div class="col-xs-7">
										{{ Form::text('deduction_'.$salary_ded[$n]->Salarayid,null,
										array('id'=>'deduction_'.$salary_ded[$n]->Salarayid,
				...
				<fieldset class="col-xs-12 mt20">
					...
					<div class="col-xs-12" style="margin-top: 0px;">
						<div class="col-xs-12 mt10 mb10">
							<div class="col-xs-5 text-right clr_blue">
								<label>{{ trans('messages.lbl_travel_exp') }}</label>
							</div>
							<div class="col-xs-7">
								{{ Form::text('Travel',null,
								array('id'=>'Travel',
									'name' => 'Travel',
									'style'=>'text-align:right;',
									'class'=>'txt ime_mode_disable form-control box38per numonly',
									'maxlength' => '15',
									'onkeypress'=>'return event.charCode >=6 && event.charCode <=58',
									'onkeyup'=>'return fnMoneyFormat(this.id,"jp");',
									'data-label' => trans('messages.lbl_travel_exp'))) }}
							</div>
						</div>
					</div>
				</fieldset>
				<fieldset class="col-xs-12 mt20">
					<legend align="left" style="width: auto !important; background-color: white; border:none !important;float: left; font-size: 15px; position: relative; margin-top: -13px;margin-bottom: 0px !important;margin-left: -5px !important;">
					<b>{{ trans('messages.lbl_totamt') }}</b></legend>
					<div class="col-xs-12" style="margin-top: 0px;">
						<div class="col-xs-12 mt10">
							...
							<div class="col-xs-7">
								<span id="totamt" data-totalamt="" class="fwb clr_blue1" style="font-size: 120%">
								</span>
								<span class="fwb ml10" style="font-size: 120%;">円</span>
								
							</div>
						</div>
						<div class="col-xs-12 mt10 mb10">
							<div class="col-xs-5 text-right clr_blue">
								<label>{{ trans('messages.lbl_sal_amt') }}</label>
							</div>
							<div class="col-xs-7">
								{{ Form::text('salamt','',array('id'=>'salamt',
									'name' => 'salamt',
									'style'=>'text-align:right;',
									...
									'onkeypress'=>'return event.charCode >=6 && event.charCode <=58',
									'onkeyup'=>'return fnMoneyFormat(this.id,"jp");',
									'data-label' => trans('messages.lbl_sal_amt'))) 
								}}
								<span id="difference_amount" style="padding-left: 13px;"></span>
							</div>
						</div>
					</div>
				</fieldset>
		...
		{{ Form::close() }}
		{{ Form::open(array('name'=>'contractempaddeditcancel', 
				'id'=>'contractempaddeditcancel',
				'url' => 'contractEmp/index?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),'files'=>true,'method' => 'POST')) }} 
				...
				<div class="modal-content">
					<div class="modal-header" style="padding: 3px !important;">
						<div class="col-md-12" style="padding-left: 15px;">
						  <h2 style="font-size:30px;" class="modal-title custom_align"> 
							{{ trans('Do You Want to Take Next Record?') }}</h2>

						</div>
					</div>
					{{ Form::open(array('name'=>'hiddenform', 'id'=>'hiddenform', 
							'url' => 'contractEmp/view?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),
					  		'method' => 'POST')) }}
					...
					<div class="modal-body" style="padding: 1px !important;">
						<center class="mt10 mb10">
							...
							<button onclick="javascript:gotoviewpage();"
								class="btn btn-danger CMN_display_block box100">
								{{ trans('messages.lbl_close') }}
							</button>
						</center>
					</div>
					{{ Form::close() }}
