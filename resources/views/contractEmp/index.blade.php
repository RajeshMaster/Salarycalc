...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$('#contractempindex input[type=checkbox]').attr('checked',false); 
  		var ht;
...
<div class="CMN_display_block" id="main_contents" style="width: 100%;">
	...
	<article id="expenses" class="DEC_flex_wrapper " data-category="expenses expenses_sub_7">
		{{ Form::open(array('name'=>'contractempindex',
							'id'=>'contractempindex',
							'url'=>'contractEmp/index?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),
							'files'=>true,
							'method' => 'POST' )) }}
		...
		<div class="col-xs-12 mt5 pm0 pull-left pl10">
			...
			<div style="display: inline-block;" class="mr10 mb10 pull-right">
				<a href = "javascript:historyTotal('{{ $request->mainmenu }}');" 
					class = "btn btn-warning" title = "History Total" 
					style = "color: white;"><span class="fa fa-history"></span>
					{{ trans('messages.lbl_conEmployee').' '.trans('messages.lbl_history').' '.trans('messages.lbl_total') }}
				</a>&nbsp;

				<a href = "javascript:transferdetailsdownload('{{ $request->mainmenu }}');" 
		...
		@if($salArr != "")
			<table id="multiadd_table" class="tablealternate mb50">
				...
				<tr>
					<th rowspan="3" class="vam sticky w_1 multiadd_th">
						{{ trans('messages.lbl_sno') }}</th>
					<th rowspan="3" class="vam sticky1 w_2 multiadd_th">
						{{ trans('messages.lbl_empno') }}</th>
					<th rowspan="3" class="vam sticky2 multiadd_th" 
						style="min-width: 205px;">{{ trans('messages.lbl_name') }}</th>
					<th rowspan="3" class="vam sticky w_1 multiadd_th" 
						style="min-width: 40px;">
						{{ Form::checkbox('checkall', 1,'',['id' => 'checkall']) }}</th>
					...
					<th rowspan="1" colspan="<?php echo $sal_det_cnt; ?>" 
						class="vam multiadd_th">{{ trans('messages.lbl_salary_det') }}
					</th>
					<th rowspan="2" colspan="1" class="vam multiadd_th" 
						style="min-width: 110px;background-color: #f39c1280 !important">{{ trans('messages.lbl_totpay_amt') }}
					</th>
					@endif
					...
					@php($sal_ded_cnt = '')
	 				...
	 				<th rowspan="1" colspan="<?php echo $sal_ded_cnt; ?>" 
	 					class="vam multiadd_th">{{ trans('messages.lbl_salary_ded') }}
	 				</th>
	 				<th rowspan="2" colspan="1" class="vam multiadd_th" 
	 					style="min-width: 110px;background-color: #f39c1280 !important">{{ trans('messages.lbl_totded_amt') }}
	 				</th>
		 			@endif
					...
					@if($tot_travel_amt != '')
 						@php($sal_trv_cnt += 1)
 						<th rowspan="2" colspan="1" class="vam multiadd_th">
 							{{ trans('messages.lbl_travel_exp') }}</th>
 						<th rowspan="2" colspan="1" class="vam multiadd_th" style="min-width: 110px;background-color: #f39c1280 !important">
 							{{ trans('messages.lbl_total') }}</th>
					@endif
		 				...
		 				@for ($i = 0; $i < count($salary_det); $i++)
		 					@if(isset($temp_salaryDetails[$salary_det[$i]->Salarayid]) && $temp_salaryDetails[$salary_det[$i]->Salarayid] != '0')
			 					<th class="vam multiadd_th">
			 						<label title="{{ (isset($salary_det[$i]->nick_name) && $salary_det[$i]->nick_name != '')?$salary_det[$i]->Name:'' }}" style="margin-bottom: 0px !important;">
