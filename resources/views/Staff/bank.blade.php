...
<article id="staff" class="DEC_flex_wrapper " data-category="staff staff_sub_2">
	{{ Form::open(array('name'=>'bankIndex',
						'id'=>'bankIndex',
						'url' => 'Staff/bankIndex?mainmenu='.$request->mainmenu.'&time='.date('YmdHis'),
						'files'=>true,
						'method' => 'POST')) }}
					...
					@for ($i = 0; $i < count($empdetails); $i++)
						<tr>
							...
							<td title="{{ empnameontitle($empdetails[$i]->LastName, $empdetails[$i]->FirstName,50) }}">
								{{ empnamelength($empdetails[$i]->LastName, $empdetails[$i]->FirstName,12) }}
							</td>
