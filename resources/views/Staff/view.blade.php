...
<article id="staff" class="DEC_flex_wrapper " data-category="staff staff_sub_1">
	{{ Form::open(array('name'=>'frmstaffview',
						'id'=>'frmstaffview',
		...
		<div class="pull-left ml5">
			...
			<!-- @if($request->resignid == "" || $request->resignid == "0")
				<div class="pull-right mr5">
					<a href="javascript:resignpage('{{ $request->mainmenu }}','{{ $request->viewid }}','1');" class="btn btn-primary box100 pull-right pr10"><span class="fa fa-close"></span> {{ trans('messages.lbl_resign') }}</a>
				</div>
				<div class="pull-right mr10">
				<a href="javascript:editview('edit','{{ $request->viewid }}');" class="pageload btn btn-warning box80 pull-right pr10"><span class="fa fa-pencil"></span> {{ trans('messages.lbl_edit') }} </a>
			</div>
			@else
					<a href="javascript:rejoinpage('{{ $request->mainmenu }}','{{ $request->viewid }}','{{ $request->resignid }}');" class="btn btn-primary box100 pull-right pr10"><span class="fa fa-close"></span> {{ trans('messages.lbl_rejoin') }}</a>
			@endif	 -->	
	...
	<fieldset>
		<div class="box60per CMN_display_block">
			...
			<!-- <div class="col-xs-12">
				...
				<div>
					{{ Form::checkbox('mailneed', 1, (isset($staffdetail[0]->mail_need)) ? $staffdetail[0]->mail_need : 1, ['id' => 'mailneed']) }}
