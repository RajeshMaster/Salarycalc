...
class Eradate{
	...
	//平成
	private static $heiseiArr=array("平成","H","4","1989/01/08","2019/04/30","平");
	//令和
	private static $reiwaArr=array("令和","R","5","2019/05/01","","令");
	//時代範囲
	private static $eraYr=array("1868","1912","1926","1989","2019");
	//現在の時代
	private static $CURRNENDO="R";
	...
	public static function geteradate($date,$type){
		...
		} elseif($substrvalue == self::$reiwaArr[1]){
			$returnkanji = self::$reiwaArr[0]."/".self::$reiwaArr[5];
		} else {
			$returnkanji =  " / ";
		}
	...
	public function getNendoFormated($nendo,$formatFlg){
		...
		if($len==1){
			...
			if(is_numeric($Ere)){
				...
				}elseif($Ere==Eradate::$reiwaArr[2]){
					$Ere=Eradate::$reiwaArr[1];
				}
			}
		...
		if($Eyear!=null){
			...
			if($formatFlg==1){//24
				...
				}elseif($Ere==Eradate::$reiwaArr[1]){
					$Ere=Eradate::$reiwaArr[2];
				}
...
/*********************西暦→和暦変換関数  開始*********************/
	...
	*********************************************************/
	...
	private static function getera($type,$date,$separator){
		...
		//5.　開始年月日：2019/05/01　元号名称：令和
		$era5 = Carbon::createFromFormat('Y/m/d',Eradate::$reiwaArr[3]);
		...
		if($curDate >=$era1 && $curDate< $era2){
			...
			$date=Eradate::geteraformatted($era,Eradate::$eraYr[2],
				  $date,$type,$separator);
		}else if($curDate>=$era4 && $curDate<$era5){
			$era =Eradate::$heiseiArr[$val];
			$date=Eradate::geteraformatted($era,Eradate::$eraYr[3],
				  $date,$type,$separator);
		}else if($curDate>=$era5){
			$era =Eradate::$reiwaArr[$val];
			$date=Eradate::geteraformatted($era,Eradate::$eraYr[4],
				  $date,$type,$separator);
		}
...
/*********************和暦→西暦変換関数  開始*********************/
	...
	*********************************************************/		
	private static function ryakushotosei ($date){
		...
		}else if($era==Eradate::$reiwaArr[1] || $era==Eradate::$reiwaArr[2]){
			$res=Eradate::getdatefromera(Eradate::$eraYr[4],$yr,$date);
		}
		return $res;		
	}
...
/*********************和暦→西暦変換関数  完了*********************/
	...
	private static function getyear ($Nendo){
		...
		if($era==Eradate::$meijiArr[1] || $era==Eradate::$meijiArr[2]){
			...
			$res=Eradate::getyearformat(Eradate::$eraYr[4],$yr);
		}
		return $res;		
	}
