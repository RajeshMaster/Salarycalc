...
class StaffController extends Controller {
	...
	//To Get The Latest Employee Details In New DataBase
				...
				foreach ($oldUserQuery as $key => $value1) {
					...
					if (is_numeric(trim($return_address))) {
						$oldAddress = Staff::fnGetAddressMB($value1->Address1);
						if (isset($oldAddress[0])) {
							$getOldUserRecordsAsArray[$j]['Address1'] = '〒'.$oldAddress[0]->pincode.' '.$oldAddress[0]->jpstate.$oldAddress[0]->jpaddress.' - '.$oldAddress[0]->roomno;
						} else {
							$getOldUserRecordsAsArray[$j]['Address1'] = '';
						}
					} else {
				    ...
				    // Start Madasamy Code 23/05/2020
					$Designation=Staff::selectdesignation($value1->Designation);
					if (count($Designation) > 0) {
					    $getOldUserRecordsAsArray[$j]['Designation']=$Designation[0]->DesignationNM;
				    } else {
				    	$getOldUserRecordsAsArray[$j]['Designation'] = "";
				    }
					$getOldUserRecordsAsArray[$j]['citizenShip']=$value1->citizenShip;
					$getOldUserRecordsAsArray[$j]['Old_ID']=$value1->Old_ID;
				    // End Madasamy Code 23/05/2020
					$j++;
				}
				// if( $g_val > $employee_count ){
					if ($getOldUserRecordsAsArray != "") {
						...
						foreach ($getOldUserRecordsAsArray as $key => $value) {
							...
							// Start Madasamy Code 23/05/2020
							$getOldUserRecordAsArray[$i]['Designation'] = $value['Designation'];
							$getOldUserRecordAsArray[$i]['citizenShip']=$value['citizenShip'];
							$getOldUserRecordAsArray[$i]['Old_ID']=$value['Old_ID'];
							// End Madasamy Code 23/05/2020
							$i++;
						}
						...
						// Start Madasamy(Import Family Details) 08/06/2020
						$oldRecords = Staff::fnGet_emp_family_dtl();
						foreach ($oldRecords as $key1 => $record) {
							$oldRecordAsArray = (array)$record;
							$result = Staff::fnUpdateOrInsert('emp_family_details',$oldRecordAsArray);               
						}
						
						Session::flash('success', 'Imported Sucessfully!'); 
						Session::flash('type', 'alert-success'); 
						//End Madasamy(Import Family Details) 08/06/2020
					} else {
	...
	function update_mail(Request $request){
		$getid = Staff::fnupdateMail($request);
		print_r($getid);exit();
	}
}