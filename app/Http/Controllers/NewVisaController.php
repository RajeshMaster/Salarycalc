...
class NewVisaController extends Controller
{
    public function index(Request $request){
        ...
        $sortarray = ['Emp_ID'=>trans('messages.lbl_empid'),
                      'LastName'=>trans('messages.lbl_empName')] ;

        $employees = NewVisa::visalist($request);

        // print_r($employees);exit;

        return view('NewVisa.index', ['emps' => $employees,
                                  'sortarray' => $sortarray,
                                  'request' => $request]);
    }
    ...
    public function view(Request $request) {
        ...
        if (!isset($request->Emp_ID)) {
          return Redirect::to('NewVisa/index?mainmenu=newvisa&time='.date('YmdHis'));
        }

        $singleview = NewVisa::singleview($request->Emp_ID);
        $citizentype = array('1'=>$msg = "Indian",'2'=>$msg = "Japanese");

        $passport = NewVisa::fnGetPassport($request);

        $educationData = NewVisa::fnGetEducation($request->Emp_ID);
        ...
        $certificate = NewVisa::fnGetCertificate($request->Emp_ID);

        $experience = array();
        if (isset($singleview[0]->Old_ID)) {

            $TRN_Val = NewVisa::singleview($singleview[0]->Old_ID);
            if (isset($TRN_Val[0]->Old_ID)) {
                $experience['DOJ_SS'] = $TRN_Val[0]->DOJ;
            } else{
                $experience['DOJ_SS'] = $singleview[0]->DOJ;
            }
        ...
        $dateDiff = abs(strtotime((isset($experience['DOJ_SS'])) ? $experience['DOJ_SS'] : '') - strtotime(date('Y-m-d')));
        if ($dateDiff > 0) {
          $experience['years'] = floor($dateDiff / (365*60*60*24));
        } else {
          $experience['years'] = '';
        }

        return view('NewVisa.view', ['singleview' => $singleview,
    ...
    public function getExperienceDet($Old_ID){
        $dojSS = array();
  ...
  // }
    ...
    public function webcampopup(Request $request) {
        return view('Visa.webcampopup',['request' => $request]);
    }
    ...
    public function rotation(Request $request) {
             $path = $request->path;
             $angle = $request->rotateval;
             $file = $request->file;
             $count = $request->filecnt;
             $ext = substr($file,-3);
             $degrees= -1*$request->rotateval;
             $filename1 = $request->file;
             $sourceName = $request->path.$filename1;
             $new_file=$sourceName;
             $filename =$request->path.$request->file;
             $rotang = $degrees;
             list($width, $height, $type, $attr) = getimagesize($filename);
              ...
              {
                 case 'image/jpeg':
                        $update = Helpers::imagecompressionforjpg($filename,$rotang,$width,$height,$new_file);
                 break;
                 case 'image/png':
                        $update = Helpers::imagecompressionforpng($filename,$rotang,$width,$height,$new_file);
                 break;
                 case 'image/gif':
                        $update = Helpers::imagecompressionforpng($filename,$rotang,$width,$height,$new_file);
                 break;
          ...
          if($update) {
            Session::flash('message', 'Image was Rotated Sucessfully!'); 
            Session::flash('type', 'alert-success'); 
          } else {
            Session::flash('type', 'Image was not Rotated!'); 
            Session::flash('type', 'alert-danger'); 
          }
          Session::flash('visadbid', $request->id );
          Session::flash('visaid', $request->visaid );
        ...
        Excel::load($template_name, function($objPHPExcel) use($request) {
          ...
          if ($aliencard[0]->Address != '') {
            ...
            if (isset($japanAddress[0])) {
              $japanAddress = '〒'.$japanAddress[0]->pincode.' '.$japanAddress[0]->jpstate.$japanAddress[0]->jpaddress.' '.$japanAddress[0]->roomno;
            }
          }
          ...
          if (isset($empArray['Old_ID'])) {
            $dojSS = self::getExperienceDet($empArray['Old_ID']);
          ...
          $expYears = round(floor($dateDiff / (365*60*60*24)));
          ...
          if (isset($empArray)) {
              ...
              $objPHPExcel->getActiveSheet()->setCellValue("G27", $japanAddress);
