...
Class ContractEmpController extends Controller {
	...
	public function contractemppopup(Request $request) {
		...
		$employeeSelect = ContractEmp::getAllEmpDetails($request);
		
		return view('contractEmp.contractemppopup',[ 'request' => $request,
											'employeeSelect' => $employeeSelect
										]);
	}

	public function empselectprocess(Request $request) {
		
		$insert = ContractEmp::InsertEmpFlrDetails($request);
		if($insert){
	...
	public function addedit(Request $request) {
		...
		if ($request->next_record == 1) {
			...
			$g_query = ContractEmp::salaryDetail($request,$request->selYear,$request->selMonth,1);
			foreach ($g_query as $key => $value) {
				if ($value->Salary == "" && $value->Deduction == "") {
					$empName = ContractEmp::fnGetEmpName($value->Emp_ID);
					if (isset($empName[0])) {
	...
	public function getlastmonthdet(Request $request) {
		...
		$request->selMonth = $date_month[1];
		$detedit = array();
		$details = ContractEmp::salarycalcplusview_tot($request);
	...
	public function getTransferedAmount(Request $request) {
		...
		$transferedamt = ContractEmp::inv_salary_main_transferedamt($request);
	...
	/**
		*
		* Transfer Details Download for Index
		* @author Sastha
	...
	*/
	...
	/**
		*
		* Payroll Excel Download for Index and History Total
		* @author Sastha
		* @return object to particular view page
		* Created At 11/03/2021
		*
